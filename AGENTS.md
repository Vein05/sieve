...
Before interpreting results, proposing experiments, or suggesting a paper
framing, read `CONTEXT.md` completely. The predecessor paper has already
published the fixed-compression/reader-scaling diagnosis. This repository is
for the new capability-conditioned SIEVE v2 paper, not for re-proving that
diagnosis or presenting SIEVE v1 as a new universal compressor.
