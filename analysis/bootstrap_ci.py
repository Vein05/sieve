...
def load_judge(run_dir: Path) -> dict[str, int]:
...
def bootstrap_delta_ci(
    sieve_scores: list[int],
    naive_scores: list[int],
    n_boot: int = N_BOOT,
) -> tuple[float, float, float]:
    """Compute bootstrap 95% CI on accuracy delta (sieve - naive). Returns (delta, lo, hi)."""
    n = len(sieve_scores)
...
def bootstrap_correlation_ci(
    ...
    n = len(x)
    from scipy import stats as sp_stats  # optional dependency, guarded by main guard
...
def main() -> None:
    ...
    for name, pts in DOMAIN_DATA.items():
        ...
        deltas_list = [p[1] for p in pts]
        r, lo, hi = bootstrap_correlation_ci(baselines, deltas_list)
